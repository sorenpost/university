...
```{R eci_gdp_figure, fig.height = 3.5, echo = FALSE, message = FALSE, warning =FALSE, fig.cap="GDP/cap (PPP, 2011 Intl \\$) and ECI"} 
library(tidyverse)
library(janitor)
library(ggpubr)
...
```{R outputTasks, fig.height = 2.5, echo = FALSE, message = FALSE, warning = FALSE, fig.cap="O-ring output by worker quality and tasks: (A) for very small declines in worker quality we see massive drops in expected output (in the example $n = 10$); (B) as complexity of production increases, the drop from potential output driven by marginal decreases in quality skyrockets. In temrs of production, this suggest that small changes in the risk (quality) of failure disproportionately punishes higher complexity producers."} 
...
Why are these results relevant to the production of more or less complex products? 

- greater gains as n increases = more complex productions will be done in higher $n$ environments.

- matching = environment matching: production chains are matched in their environment. That is, 
