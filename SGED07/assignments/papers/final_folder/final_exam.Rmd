...
If the size of the market limits the specialization of firms and workers, the globalization of labor- and input-markets should facilitate broad economic wealth creation. When all countries can exploit the global markets, why then have national differences in the gross domestic product (GDP) per capita skyrocketed during the last two hundred years [@pritchett_divergence_1997]? Despite 50 years of unprecedented international connectivity, international trade, and globalisation (and some notable growth spurts), the data show that developing countries (as a group) are not catching up to more advanced economies [@johnson_what_2020].

The literature on economic complexity provides one possible answer. If some spill-over effects from the individual activities that arise from  specialization - like property rights, tacit know-how, infrastructure, regulation - cannot be imported, they need to be present in the local economy. The productivity of a country then lies in these non-tradable "economic capabilities", and the differences between countries owe (partly) to their number, complimentarity, and interactions of these capabilities [@hidalgo_product_2007; @hausmann_atlas_2013]. 

While competing methods exist [@tacchella_new_2012; @hidalgo_building_2009; @inoua_simple_2016],  approaches to quantifying these capabilities share a common conceptual grounding. Given the difficulties in defining and measuring discrete capabilities, researchers have taken an agnostic approach to specific nature of capabilities. Instead, 

![The tripartite graph (left) represents the theoretical model: countries (C) can make the products (P) their capabilities (A) allows them to. The bipartite graph (right) is what we observe in the trade data: countries export a set of products, and from this set of products, we infer their capabilities. \label{fig:network-fig} ](/home/post/university/SGED07/assignments/papers/final_folder/network-fig-ws.png)

We can leverage this reflecting information to refine the information on the sophistication of products and countries. For example, in figure \ref{fig:network-fig} every country can produce product three. This suggests that the capabilities required to produce it are ubuiquitos. In addition, we can see that the only product country three can make is the one every country produces. This tells us that country three does not have a sophisticated capability-base. In contrast, country one can produce all products including product one, which it is the only one that can produce. This again suggests that a) country one possess a wide range of capabilities, and b) that product one is a complex product and require many or rare capabilities to produce.

```{R eci-gdp-fig, fig.height = 3.5, echo = FALSE, message = FALSE, warning =FALSE, fig.cap="\\label{fig:eci-gdp-fig}Simple best fits on GDP/cap (PPP, 2011 Intl \\$) by Economic Complexity Index (ECI). Data on resource rents is from @the_world_bank_world_2020, GDP/cap is from @the_world_bank_world_2020-1 and ECI is from @simoes_economic_2011. All observations are from 2010."} 
library(tidyverse)
library(janitor)
library(ggpubr)
...
This framework has proven to be a strong predictor of economic performance. Figure \ref{fig:eci-gdp-fig} shows the very robust relationship between country-level economic complexity and GDP per capita (PPP). Since natural resources are a product of geographical luck rather than productive know-how, I separate economies with more a larger than 10% of resource rents as share of total GDP. @hausmann_atlas_2013 shows how the deviations the observed economic complexity of economies and their GDP/cap is a strong predictor of economic growth, suggesting that they converge to the sophistication of their capabilities (that is, countries below the trend line growth fast, while countries above slows down). Additionally, using only two variables - a measure of economic complexity and GDP/cap - @tacchella_dynamical_2018 constructs a model of 5-year predictions of economic growth, beating the best-in-class predictions by the IMF (whose analysis is country-calibrated and uses hundreds of variables). Not only does aggregate complexity matter: economies moving into more complex products are more egalitarian [@hartmann_linking_2017-1] and less carbon-intensive [@can_impact_2017].

The economic complexity at the aggregate level is the outcome of a myriad of historical conditions, firm decisions, and micro-level interactions. These foundations of economic sophistication are not very well understood has been 
