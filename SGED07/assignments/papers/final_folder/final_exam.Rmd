...
```{R eci_gdp_figure, fig.height = 3, echo = FALSE, message = FALSE, warning = FALSE, fig.cap="GDP/cap and ECI"} 
library(tidyverse)
library(janitor)
library(ggpubr)
...
```{R outputTasks, fig.height = 2.5, echo = FALSE, message = FALSE, warning = FALSE, fig.cap="O-ring output by worker quality and tasks: (A) for very small declines in worker quality we see massive drops in expected output; (B) as complexity of production increases, the drop from potential output driven by marginal decreases in quality skyrockets."} 
...
```{R disr_figure, fig.height = 4, echo = FALSE, message = FALSE, warning = FALSE, fig.cap="\\label{fig:figs}Production disruptions, complexity, and GDP"} 
