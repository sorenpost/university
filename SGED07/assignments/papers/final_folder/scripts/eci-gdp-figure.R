...
lm_fit <- lm(gdp_cap_ln ~ hs_eci + perc_rent_of_gdp, data = col2010 %>% filter(rents_10 == "below 10%"))
predicted_df <- data.frame(pred_gdp_cap_ln = predict(lm_fit, col2010), hs_eci = col2010$hs_eci)
...
p_eci <- ggscatter(col2010,
 ...
 ggpar(p_eci, legend.title = "Resource rents as % of GDP:", font.legend = 12, font.tickslab = 10, font.x = 12, font.y = 12)
