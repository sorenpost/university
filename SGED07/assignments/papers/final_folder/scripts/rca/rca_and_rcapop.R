...
hs4_ref <- vroom("/home/post/university/SGED07/assignments/papers/final_folder/data/hs61996/year_origin_hs92_4.tsv", delim = "\t")

hs4_ref %>%
  filter(year == 2010) %>%
  mutate(
    iso3 = str_to_upper(origin),
    export_value = as.numeric(export_val)
  ) %>%
  select(year, iso3, hs92_4 = hs92, export_value, export_rca)
...
country_total_export <- exports %>%
  group_by(iso3, year) %>%
  summarise(
    country_tot_exp = sum(export_value)
  )
...
exports <- exports %>%
  ...
  mutate(
    rca = (export_value / country_tot_exp) / (global_exp_in_prod / global_total_export)
  ) %>%
  mutate(
